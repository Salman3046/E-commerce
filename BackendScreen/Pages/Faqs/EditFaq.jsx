...
import { getSingleFaq, updateFaq } from "../../../Services/Actions/faqAction";
...
const EditFaq = () => {
  ...
  const { faq } = useSelector((state) => state.faqData);
  ...
  const submitInventory = (ev) => {
    ...
    if (!inventory.question)
      ...
      dispatch(updateFaq(inventory, id, userDataFromLocal?.token));
      setInventory({
        question: "",
        answer: "",
        status: "",
      });
      toaster({ type: "success", content: "Faq Edit Successfully" });
      navigate("/dashboard/faqs");
    }
  };

  useEffect(() => {
    dispatch(getSingleFaq(id, userDataFromLocal?.token));
    window.scrollTo(0, 0);
